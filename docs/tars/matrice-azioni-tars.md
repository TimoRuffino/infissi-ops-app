...
| Lettura del contratto PDF (piano 3, 04-05/09/2026) | Nessuno strumento Tars: la lettura resta un'azione umana dal dialog «Leggi il contratto» della commessa (`server/routers/estrazioniContratto.ts`, dietro `FLAG_CONTRATTO_ESTRAZIONE`+`FLAG_LIMITI`). La chiamata al modello passa comunque dal provider governato (`creaProviderPerRun`, classe di costo `document_intelligence`): è la PRIMA consumatrice reale di questa classe, definita nel catalogo dei costi ma senza consumatore fino a questo piano. |
