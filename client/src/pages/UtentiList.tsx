...
import { Plus, Pencil, Trash2, Users, Shield, Eye, EyeOff, KeyRound } from "lucide-react";
          ...
          <Card key={u.id} className={`transition-all ${!u.attivo ? "opacity-50" : ""}`}>
            ...
            <CardContent className="pt-0">
              <div className="flex items-center justify-between">
                <Badge className={`text-[10px] ${RUOLO_COLORS[u.ruolo] ?? ""}`} variant="secondary">
                  <Shield className="h-2.5 w-2.5 mr-1" />
                  {RUOLI.find((r) => r.value === u.ruolo)?.label ?? u.ruolo}
                </Badge>
                <Button
                  variant="ghost"
                  size="sm"
                  className={`h-6 text-[10px] ${u.attivo ? "text-green-600" : "text-red-600"}`}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="grid gap-3 py-2">
            ...
            <Button
              onClick={() => {
                if (!form.nome || !form.cognome || !form.email || !form.password || form.password.length < 4) return;
                createUtente.mutate({
                  nome: form.nome,
                  cognome: form.cognome,
                  email: form.email,
                  telefono: form.telefono || undefined,
                  ruolo: form.ruolo as any,
