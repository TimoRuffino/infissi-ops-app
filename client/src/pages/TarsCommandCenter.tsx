...
function questionText(plan: any): string {
  const data = plan.currentStepData;
  if (!data || typeof data !== "object")
    return "Tars ha bisogno di un dato per continuare.";
  return String(
    data.question ??
      data.domanda ??
      "Tars ha bisogno di un dato per continuare."
  );
}
...
function PlanWorkspace({
  ...
  const respond = trpc.tars.plans.respond.useMutation({
    onSuccess: () => {
      toast.success("Risposta registrata. Tars riprende da qui.");
      utils.tars.commandCenter.invalidate();
    },
    onError: error => toast.error(error.message),
  });
  const plans = [...snapshot.waitingQuestions, ...snapshot.activePlans];
  if (
    !plans.length &&
    !snapshot.waitingApprovals.length &&
    !snapshot.blockedCases.length
  )
    return null;
  return (
    <section className="border-y bg-card/40">
      <div className="flex items-center justify-between gap-3 py-3">
        <div>
          <h2 className="text-sm font-semibold">Obiettivi in corso</h2>
          <p className="text-xs text-muted-foreground">
            Tutto ciò che Tars sta completando con te
          </p>
        </div>
        {snapshot.waitingApprovals.length > 0 && (
          <Button size="sm" variant="outline" onClick={onOpenProposals}>
            Da approvare · {snapshot.waitingApprovals.length}
          </Button>
        )}
      </div>
        ...
        {plans.map((plan: any) => (
          <div key={plan.id}>
            ...
            {plan.status === "waiting_user" && (
              <div className="mb-3 ml-6 space-y-2 border-l-2 border-primary/30 pl-3">
                <p className="text-sm font-medium">{questionText(plan)}</p>
                <div className="flex max-w-xl gap-2">
                  ...
                  <Button
                    size="sm"
                    disabled={
                      respond.isPending || !(answers[plan.id] ?? "").trim()
                    }
                    onClick={() =>
                      respond.mutate({
                        planId: plan.id,
                        stepKey: plan.currentStep,
                        expectedVersion: plan.version,
                        response: (answers[plan.id] ?? "").trim(),
                      })
                    }
                  >
                    Continua
      ...
      {snapshot.blockedCases.length > 0 && (
        <div className="flex flex-wrap gap-2 border-t py-3">
          {snapshot.blockedCases.map((item: any) => (
            <Link
              key={item.id}
              href={item.link}
              className="inline-flex min-h-8 items-center rounded-md border px-2 text-xs hover:border-primary/35"
            >
              In attesa: {item.title}
            </Link>
          ))}
        </div>
      )}
...
function CommandCenterLoading() {
  return (
    <div
      className="space-y-3"
      aria-label="Caricamento priorità Tars"
      role="status"
    >
        ...
        <div className="flex min-h-32 items-center gap-3 rounded-lg border border-dashed px-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Le priorità certe restano attive. La direzione può riattivare
              l'analisi AI dalle integrazioni.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-lg border bg-[image:var(--gradient-soft)] px-4 py-4 sm:px-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          {snapshot.status === "degraded" && (
            <Badge
              variant="outline"
              className="w-fit gap-1 border-warning/40 text-warning-foreground"
            >
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Ordinate per urgenza, impatto e confidenza
          </p>
        </div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {priority.evidence.map((evidence: any) => {
                      ...
                      return href ? (
                        ...
                        <span
                          key={`${evidence.type}:${evidence.id}`}
                          className="inline-flex min-h-8 items-center rounded-md border px-2 text-xs text-muted-foreground"
                        >
                          {evidence.label}
                        </span>
                      );
                    ...
                    <Button
                      className="ml-auto"
                      size="sm"
                      onClick={() => onOpenProposals(priority.proposalId)}
                    >
                      Valuta
                      <ArrowRight className="h-4 w-4" aria-hidden="true" />
                    </Button>
...
export default function TarsCommandCenter() {
  ...
  const utils = trpc.useUtils();
  const snapshot = trpc.tars.commandCenter.get.useQuery(
    { limit: 12 },
    { retry: 1 }
  );
  ...
  const openProposal = (proposalId?: number | null) => {
    ...
    if (proposalId != null) {
      window.setTimeout(() => {
        document.getElementById(`tars-proposta-${proposalId}`)?.scrollIntoView({
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => {
      document
        .querySelector(`[data-plan-id="${CSS.escape(planId)}"]`)
        ?.scrollIntoView({
          behavior: window.matchMedia("(prefers-reduced-motion: reduce)")
            .matches
            ? "auto"
    ...
    <div className="min-w-0 space-y-4 pb-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          ...
          >
            <RefreshCw
              className={cn("h-4 w-4", snapshot.isFetching && "animate-spin")}
              aria-hidden="true"
            />
            Aggiorna
          </Button>
          {direzione && (
            <Button
              ...
              disabled={
                !config.data?.attivo ||
                !config.data?.auditProcessiAttivo ||
                audit.isPending
              }
      ...
      <div className="grid grid-cols-2 overflow-hidden rounded-lg border bg-card lg:grid-cols-4 lg:divide-x">
        <Metric
          ...
          detail="proposte aperte"
        />
        <Metric
          icon={ShieldCheck}
          label="Doppioni evitati"
          value={data?.metrics.duplicateAvoided ?? 0}
          detail="ultimi 30 giorni"
        />
        <Metric
          icon={Sparkles}
          label="Cache prompt"
          value={`${data?.metrics.cacheReadPercent ?? 0}%`}
          detail={`${data?.metrics.toolCacheHits ?? 0} letture riusate`}
        />
        <Metric
          icon={Clock3}
          label="Ultima analisi"
          value={relativeDate(data?.metrics.lastRunAt)}
          detail={
            data?.metrics.failedRuns
              ? `${data.metrics.failedRuns} errori recenti`
      ...
      <Tabs value={tab} onValueChange={changeTab}>
        <TabsList
          className={cn(
            "grid h-auto w-full",
            direzione ? "grid-cols-5" : "grid-cols-4",
            "sm:w-fit"
          )}
        >
          <TabsTrigger
            value="oggi"
            className="min-h-10 min-w-0 gap-1 px-2 text-xs sm:px-3 sm:text-sm"
          >
            ...
            <div className="flex min-h-64 flex-col items-center justify-center gap-3 rounded-lg border border-destructive/30 bg-destructive/5 px-5 text-center">
              ...
              <div>
                <p className="font-semibold">
                  Tars non riesce a preparare il brief
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
