...
decisioni d'esecuzione, diciassette, sono nella sua §2-bis); piano in 9 task:
...
finché la PR #7 non è fusa, la PR del WS4 conterrà anche il WS3. Il branch
**non è su `main`** e `FLAG_MULTI_AZIENDA` resta com'è: il merge è una
...
**Verificato:** `pnpm check`, `pnpm test` e `pnpm build`; Postgres vero per il
control plane e per il ledger dei costi con due aziende; interfaccia a 1440 e
390 col login demo. **Non verificato:** nulla distribuito, nessuna prova su
Railway, nessun pagamento reale.
