...
La grafia definitiva è quella a due o, fissata l'08/09/2026 insieme al
dominio **wyndoor.com**; i verbali datati 07/09 portano la forma a una o
scelta quel giorno, e non vengono riscritti perché sono cronaca. Il nome
ancora precedente resta leggibile nei documenti anteriori e nel nome del
servizio Railway, che va rinominato sulla piattaforma. Marchio e perimetro:
...
- Ortogonali nei dati, uguali per chi cerca: dal 10/09/2026 `/archivio` mostra sia le soft‑archiviate sia le commesse a `stato = "archiviata"`, e `commesse.restore` sa uscire da entrambe (§22.0, §22.1‑bis).
...
### 8.1 Tipi documento
`preventivo, contratto, misure, fattura, nota_credito, conferma_ordine,
ddt_consegna, ddt_posa, ddt_finale, saldo, foto, documento_identita, visura,
planimetria, certificazione, pratica_fiscale, pratica_edilizia,
asseverazione, scheda_tecnica, disegno, dichiarazione_conformita, garanzia,
verbale_posa, assistenza, contabile_pagamento, polizza, delibera_condominio,
altro`.

Gli ultimi dodici sono dell'08/09/2026 (mandato della direzione: «vanno
aggiunti altri tipi di doc caricabili sulle commesse»): erano i fogli che
finivano tutti in «altro» e poi non si ritrovavano — pratiche fiscali ed
edilizie, asseverazioni e cessioni del credito, schede tecniche, disegni e
computi, dichiarazioni di conformità, garanzie, verbali di posa, rapporti di
assistenza, contabili di pagamento, polizze e delibere condominiali.
...
- Dall'08/09/2026 il nome è **`{Tipo} {cliente} {AAAA-MM-GG}.{ext}`**
  (`nomeDocumentoDaTipo` in `shared/docTipi.ts`; scelta della direzione fra
  tre convenzioni). La data è quella del DOCUMENTO — per un allegato, il
  giorno in cui il messaggio è arrivato — non quella del caricamento: una
  conferma di tre settimane fa non si chiama come oggi.
- La regola vale per **tutti i tipi**, comprese foto e documenti d'identità:
  con la data nel nome due fogli dello stesso tipo non si accavallano più.
  Prima erano esentati (26/08/2026) proprio perché senza data collidevano.
  Unica eccezione: `altro`, che non ha un tipo da cui prendere il nome e
  conserva quello originale.
- Se `keepNome === true` (usato dai preventivatori), il nome viene preservato e solo dedupato.
- Il documento ricorda il nome che il file aveva quando è arrivato
  (`nomeOriginale`): serve a ritrovarlo e serve alle euristiche che leggono
  il numero d'ordine dal nome («Ordini_di_Vendi_1601814(1).pdf»), che
  altrimenti la rinomina avrebbe accecato.
...
- Dall'08/09/2026 l'anteprima dei **messaggi** usa lo stesso componente
  (`components/documenti/AnteprimaFile`): PDF nel riquadro, immagini a
  schermo, video e vocali con i loro comandi, «Apri in una scheda» sempre.
  I byte si chiedono una volta sola e si tengono come blob; quando il file
  non c'è più — Meta scarta i media WhatsApp dopo circa trenta giorni, una
  casella può essere stata staccata — il riquadro dice cosa è successo con
  le parole del server, invece di restare bianco.
...
- **Foto HEIC/HEIF** (terza tornata, stesso giorno): l'iPhone salva così, e
  né tesseract, né il modello, né i browser aprono il formato — una conferma
  fotografata in HEIC finiva «non leggibile». Si converte in JPEG in
  memoria, una volta, in testa alla cascata di lettura
  (`server/documenti/heic.ts`, `heic-convert`: libheif in WebAssembly,
  nessun pacchetto di sistema, import pigro alla prima foto) e nelle
  anteprime, dove la pagina 1 è la conversione salvata nello storage. Un
  file chiamato HEIC con dentro un JPEG non si converte; un HEIC corrotto è
  «illeggibile» con il motivo, senza chiamare OCR né modello. Lettura costo
  1.10.0: le conferme fotografate finite «non leggibili» si rileggono,
  nessun costo cambia. La fixture dei test è una foto HEIC vera fatta con
  `sips` (macOS): dove manca, quei test si saltano.
- **Fuori dalla prima versione**: posizione grossolana chiesta al modello,
  coda unica «Da verificare».
...
### 22.0 Le due forme di «archiviata» (10/09/2026)
Una commessa esce di scena in due modi, e per chi la cerca sono la stessa cosa:
1. **Soft‑archive** — `archivedAt` valorizzato, ortogonale allo stato (§7.3).
2. **Chiusura del flusso** — `stato = "archiviata"`, l'undicesimo e ultimo passo della state machine (§7.1).

Entrambe la tolgono dal Board, dal Planning e dalle aggregazioni. Quindi
**entrambe compaiono in `/archivio` ed entrambe si ripristinano da lì**.

Prima di questa regola una commessa portata a `stato = "archiviata"` finiva in
un vicolo cieco: fuori dal Board, assente da `/archivio` (che leggeva solo
`archivedAt`), senza pulsante «Ripristina» (mostrato solo con `archivedAt`) e
senza «Avanza» (escluso sull'ultimo stato). L'unica traccia restava la riga in
`/commesse` col badge «Archiviata». In produzione erano 15 commesse.

### 22.1 Soft‑archive
- Endpoint `commesse.archive(id)`: imposta `archivedAt = ISO now`.
- Lo stato (`stato`) e i collegamenti (file, aperture, interventi, prodotti) **non vengono toccati**.

### 22.1‑bis Ripristino
`commesse.restore(id)` riporta la commessa fra quelle attive, qualunque sia la
forma dell'archiviazione:
- `archivedAt` valorizzato → azzerato; lo stato resta quello che era.
- `stato = "archiviata"` → la commessa **torna a `interventi_regolazioni`**
  passando da `eseguiTransizioneCommessa` (origine `router`): la transizione
  entra nel registro, `dataChiusura` viene azzerata dal cleanup all'indietro
  (§7.2) e il passaggio è annullabile come ogni altro. Il gate documentale non
  ferma mai una transizione all'indietro, ma la capability
  `commessa.change_state` sì: riaprire una commessa chiusa non è il flag
  reversibile che tutti possono premere.
- Entrambe insieme → un solo ripristino le toglie tutte e due, in un commit
  unico (`archivedAt` viaggia nella patch della transizione).
- Commessa già attiva → nessun effetto.
...
- Le commesse a `stato = "archiviata"` restano nello scope `exclude` di
  `commesse.list`: ricerca, scheda cliente, post‑vendita e Tars devono
  continuare a vedere i lavori chiusi. Le tolgono dalla loro vista i consumatori
  che devono (Board §11.6, Dashboard, Planning, Magazzino, Fornitori, Pagamenti).
- Lo scope `only` è **l'archivio**: `archivedAt` valorizzato **oppure**
  `stato = "archiviata"`. Ogni commessa resta raggiungibile dal link diretto
  `/commesse/:id`.

### 22.3 Pagina Archivio
- Lista con search per codice/cliente/città/indirizzo.
- Per ogni riga: codice, stato originale (badge), priorità, cliente, indirizzo, data apertura, pulsanti **Ripristina** e **Apri scheda**.
- La data dichiara la forma: «Archiviata: gg/mm/aaaa» (`archivedAt`) oppure «Chiusa: gg/mm/aaaa» (`dataChiusura`).
- Conferma esplicita prima del ripristino, con il testo che dice cosa succede: la chiusa avverte che verrà riaperta a «Interventi / Regolazioni» e che la data di chiusura viene azzerata.

### 22.4 Pulsanti nella scheda commessa
- **Archivia** (se non archiviata) — conferma non‑destructive.
- **Ripristina** — presente in **entrambe** le forme.
- Banner in alto: «Commessa archiviata» (soft‑archive) o «Commessa chiusa» (stato terminale), ognuno con la sua spiegazione.
- In entrambe le forme la scheda non mostra lo StatusRail né «Avanza»; il badge «Archiviata» dell'header resta al solo soft‑archive, perché per la commessa chiusa lo dice già il chip dello stato.
...
Le tabelle PostgreSQL `tenants`, `tenant_eventi` (append-only, un trigger
rifiuta `UPDATE`/`DELETE`) e `tenant_comandi` sono il control plane del
multi-azienda (WS1, §60.9), fuori dal KV store; `server/tenants/repository.ts`
è l'unico file che vi scrive. Lo schema esiste già con `FLAG_MULTI_AZIENDA`
spento (`avviaTenants()` gira comunque dopo il bootstrap), ma resta vuoto:
il seed del tenant 1 parte solo alla prima accensione dell'interruttore.
...
- **v5.89 (09/09/2026)** - **«Modifica azienda» dal pannello piattaforma** (§60.13 addendum). Dalla scheda di un'azienda si correggono ragione sociale, slug, note; i sei campi di fatturazione (P.IVA, codice fiscale, sede legale, email amministrativa, PEC, codice SDI, colonne additive su `tenants`); la sede predefinita (nome, città); il proprietario (nome, cognome, email, telefono). Due comandi nuovi (`modifica_tenant`, `modifica_proprietario`) eseguiti subito come gli altri del pannello; eventi `tenant_modificato` (solo i campi davvero cambiati), `slug_cambiato`, `proprietario_modificato`. Il tenant 1 non cambia slug, l'email del proprietario resta unica sull'installazione, e se cambia prima che il proprietario accetti l'invito questo si annulla e si riemette **solo per lui** — un'azienda con più proprietari non tocca l'invito degli altri; `accettaInvito` rifiuta comunque un link la cui email non coincide più con quella corrente. Dialogo «Modifica azienda» nella scheda, quattro pannelli (Azienda, Fatturazione, Sede, Proprietario), una sola conferma password che manda al più due mutation in fila. **Verificato:** `pnpm check` pulito, `pnpm test` 355 file passati + 15 saltati (3864 test passati + 88 saltati), `pnpm build` riuscito, i quattro `*.pg.test.ts` di `server/tenants` 4 file / 26 test su Postgres vero. Su branch `feature/modifica-azienda`, PR aperta. **Nel frattempo, lo stato vero di WS5 e WS6** (le voci v5.87/v5.88 qui sotto li davano non ancora in produzione): **WS6** «pannello piattaforma» è fuso in `main` (PR #9, merge `cff8ef0`) e in produzione dal 09/09/2026 sera, con la prima azienda pilota creata dal pannello (tenant 2, invito accettato); **WS5** «collegamento delle integrazioni» è fuso in `main` (PR #10, merge `d896101`) e in produzione dalle 20:22 dello stesso giorno, seguito la notte stessa da una fix wave in tre PR su `main` — #11 (`cf0475c`, cornice/scorrimento), #12 (`fb1970b`, WhatsApp di piattaforma), #13 (`609360c`, vista essenziale delle integrazioni) — già raccontata in `handoff.md` e `CLAUDE.md`.
- **v5.88 (09/09/2026)** - **WS5 «collegamento delle integrazioni in self-service» sul branch** (§60.14). I 13 task del piano sono stati eseguiti inline, senza revisione per task, su un altro worktree (`feature/ws5-collegamento-integrazioni`, base il WS4 a metà `873b6c6` poi fuso col WS4 vero `e38874c`); quel branch è stato fuso in `feature/ws5-integrazioni-su-main`, nato da `main` @ `cff8ef0` — che contiene già WS1, WS2, WS3, WS4 **e WS6** (§60.13, merge `cff8ef0`/PR #9: il WS5 resta l'unico workstream fuori da `main`) — con merge `c139b83`. Una cornice unica (`server/integrazioni/contratto.ts`) fa rispondere sei integrazioni alle stesse tre domande — a cosa sono collegata, come mi collego, cosa si è rotto — con **cinque** adattatori attivi (`fic`, `email`, `whatsapp`, `backup`, `agente`: `calendario` resta un tipo senza adattatore, fase 4 non partita) che avvolgono i router esistenti senza riscriverli; `stato()` non chiama mai il fornitore, `verifica()` è la prova viva in cache 60 s per azienda. Le credenziali OAuth (Fatture in Cloud, Google) e l'app Meta di WhatsApp diventano di **piattaforma** — il cliente autorizza il proprio account ma non apre mai un'area sviluppatori — con l'override per sede di WhatsApp come via di fuga, valido solo come terna intera. Percorso di attivazione guidato dopo l'invito (WS6), «Salta» sempre disponibile. La revisione dell'intero branch (`cff8ef0..c139b83`) ha trovato 2 Critical e 11 Important, chiusi in un'unica fix wave (8 commit, `2901017`…`0d6d70d`): **C1**, la cache di `verifica()` senza l'azienda nella chiave serviva l'esito di un cliente a un altro; **C2/W2**, il redirect OAuth del Drive ricostruito in due modi diversi nello stesso giro (`redirect_uri_mismatch`) ora viaggia nel payload dello `state` come già fa Fatture in Cloud; **W3**, «Prova il collegamento» e il percorso di attivazione resi raggiungibili subito; **W4**, il modulo manuale di WhatsApp resta sempre dentro una Diagnostica invece di sparire al deploy. Cinque minori restano rimandati ai documenti (spec §2-bis «Ancora aperti»): soggetto FiC con l'id non il nome, `agente.verifica()` che non interroga davvero OpenAI, tre ripieghi `?? 1`, il ramo service-account del Drive, `getCfg` che scrive da una query nominalmente di sola lettura. **Verificato:** `npx vitest run shared/brand.test.ts` 4/4, `pnpm check` pulito, `pnpm test` 351 file passati + 15 saltati (3759 test passati + 86 saltati), `pnpm build` riuscito, `pg.test` su `perf-pg-test` 14 file / 71 test passati. **Non verificato:** nessun giro OAuth reale né Embedded Signup con l'app di piattaforma, nulla distribuito su Railway con le variabili nuove (`GOOGLE_OAUTH_REDIRECT_URI`, la terna `WHATSAPP_*`, `WHATSAPP_VERIFY_TOKEN`: da impostare dopo il deploy). Non su `main`, non in produzione.
- **v5.87 (09/09/2026)** - **WS6 «pannello piattaforma» sul branch** (§60.13). Lo stesso giorno in cui WS3 e WS4 sono andati su `main` (PR #8, merge `37c1889`) e `FLAG_MULTI_AZIENDA` è stato acceso in produzione (09:54), nasce da quel `main` `feature/ws6-pannello-piattaforma`: 10 task committati (`9dc4c54`…`a606515`) danno alla direzione una sezione `/piattaforma` che gestisce tutte le aziende dall'app — elenco con stato/abbonamento/spazio/Tars/worker/backup, «Nuova azienda» con invito via email al proprietario (password inutilizzabile, link monouso di 7 giorni, Resend o link da copiare senza chiave), sospensione/riattivazione, le sette azioni sull'abbonamento, proprietari, ricalcolo dello spazio e ripristino degli archivi, eventi e comandi come traccia. Chi amministra è un utente attivo del tenant 1 con l'email in `PLATFORM_ADMIN_EMAILS` — **deviazione dichiarata** dal design madre (identità separata con MFA): nessun secondo login, resta la conferma password (stesso limitatore del login, estratto in `server/_core/limiteTentativi.ts`) sulle azioni sensibili. Ogni scrittura è un comando in `tenant_comandi` con `richiesto_da = piattaforma:<email>`, eseguito **subito** dalla stessa funzione del giro dei 30 secondi (ricalcolo dello spazio e ripristino degli archivi restano in coda). Tredici decisioni d'esecuzione (due pre-volo, R1-R8 durante i task, R9-R11 dalla revisione finale del branch intero) nella spec §2-bis: fra le più rilevanti, il claim mancante nel repository in memoria per `eseguiComandoSubito` (R2; su Postgres il comportamento era già corretto), la guardia dell'amministratore senza controllo `loginMethod` (R3; non sfruttabile oggi, OAuth spento), un secondo comando lungo che faceva perdere polling e toast a un primo già in corso (R8), l'invito diventato azione sensibile con il link che esce solo se la posta non è partita (R9) e la **porta chiusa a interruttore spento** — a `FLAG_MULTI_AZIENDA=off` il contesto fissa `tenantId = 1` per chiunque, quindi un utente di un'altra azienda non fa più login e non ha sessione (R10). **Verificato:** `npx vitest run shared/brand.test.ts` 4/4, `pnpm check` pulito, `pnpm test` 339 file passati + 15 saltati (3665 test passati + 86 saltati), `pnpm build` riuscito, `pg.test` su `perf-pg-test` 14 file / 71 test passati. **Non verificato:** `/piattaforma` con una sessione reale (i Task 8 e 9 hanno verificato l'interfaccia a 1440 e 390 con un link tRPC finto e dati seminati), nessuna consegna reale via Resend, nulla distribuito su Railway. Non su `main`, non in produzione.
- **v5.86 (09/09/2026)** - **WS4 «abbonamenti» finito sul branch e `main` rifuso** (§60.12; §60.11 per il WS3 che il branch contiene). Ogni azienda ha un abbonamento nel control plane (prova di 30 giorni, omaggio, proroga, avvisi 7/3/1, insoluto e sola lettura dopo 7 giorni, worker ogni 6 ore); la quota storage e il budget Tars per azienda **bloccano** dopo la tolleranza; il tenant 1 è avvisato ma mai bloccato (R16); l'abbonamento è la fonte di verità e un tenant riattivato a mano con il contratto sospeso viene risospeso al giro successivo (R15); la migrazione dei record legacy rispetta il blocco e si ferma subito col messaggio (R17). Provider di pagamento «nessuno» dietro l'adattatore; venti decisioni d'esecuzione nella spec §2-bis. Fusione di `main` 5.85→5.65 il 09/09 (numerazioni parallele spiegate nell'intestazione). Non su `main`, non in produzione; `FLAG_MULTI_AZIENDA` spento.
...
- **v5.47 (06/09/2026)** - **Studio sui dati reali, fase 5: il corpus del Drive NAS** (spec §11; commit `2953cd4`, `abc0923`). Dalle 311 cartelle del Drive con foglio limiti, contratto e fattura: 229 fogli 2022-24 raccolti, **134 riprodotti al centesimo**, 71 nuovi in fixture (63 erano copie dei fogli locali) → **148 casi d'oro**. Dei 95 non riprodotti, 62 hanno misure con i decimali (foro + alette: il CRM lavora in mm interi; il raccoglitore ora mette `salta` H9 e arrotonda invece di troncare), 19 gli stessi decimali in un altro blocco, 7 lavori una voce DEI di riga diversa (da indagare), 2 illeggibili. 34 contratti 2023-24 letti con la lettura visiva (corsa fermata dalla direzione): 32 pattuiti trovati, nessun crash, 8 sanificazioni; sulle righe la verità dei fogli 2023-24 non vale (misure del foro, +90/+45 mm) — solo 7 righe su 127 sono mancanze vere del modello. Lezione: il lettore si giudica con verità scritte dal contratto, non dai fogli limiti del 2023-24. Suite: 241 file passati e 9 saltati, **2.615 test passati e 53 saltati**; `pnpm check` e build puliti.
...
Mandato della direzione del 07/09/2026: «crea la pagina fornitori in cui
automaticamente per ogni fornitore vengono archiviate tutte le conf. ordine
e le comunicazioni in automatico da Tars; da lì poi deve analizzare la conf.
ordine e capire di quale commessa è, e se non lo capisce deve dirlo e va
collegata a mano, così che una volta collegata compaia nella commessa e da
lì si ricavi poi il costo fornitore, stessa cosa per il prodotto in
magazzino».

### 36-bis.1 Che cos'è
Un **indice sulle comunicazioni**, non una seconda copia dei file: le
conferme vivono già come allegati delle mail e nello storage. L'archivio
(`server/fornitori/archivio.ts`, store `fornitori_archivio`) registra per
ogni fornitore quale conferma è arrivata, che cosa ha capito la lettura e
dove è finita. Tre stati soli:

| Stato | Significato |
|---|---|
| `da_collegare` | la commessa non è certa: il motivo è scritto e decide una persona |
...
3. **decisione** — commessa **unica** e viva: la conferma entra da sola nel
   ...
   fa nascere costo fornitore e consegna a magazzino. In tutti gli altri
   casi la voce resta `da_collegare` **con il motivo**: «il testo cita più
   commesse», «non cita nessuna commessa viva», «il file non si legge».

### 36-bis.3 Collegamento a mano
Dalla pagina, su una voce da collegare: i **candidati che il testo nomina**
(un click per ciascuno) e la ricerca libera fra le commesse vive. Il
collegamento (`fornitori.archivio.collega`, direzione o amministrazione
come per «È di questa commessa» del registro conferme) archivia il file nel
fascicolo con `origine: "fornitori"`, collega la mail se era libera, e
restituisce che cosa è nato: il costo imponibile e la consegna. Una voce già
collegata non si sposta da qui: si toglie dal fascicolo della commessa.
Altre azioni: **Rileggi** (dopo una correzione dell'estrattore), **Non è da
...
interfaccia (D1 sospesa dalla direzione). L'archivio non manda mail e non
crea ordini: legge, dice e collega.
...
Il thread e cronologico e paginato. Dall'08/09/2026 (mandato della direzione:
«devo poter vedere l'anteprima dei file inviati su whatsapp» e «devo poterli
collegare alle commesse, sia su whatsapp che sulle mail») **gli allegati sono
allegati, non metadati**: le foto si vedono nella bolla, ogni file ha
anteprima, download e «Archivia nel fascicolo» (§51.9). Il workspace resta di
sola lettura per quanto riguarda WhatsApp: nessun invio di messaggi o media,
nessuna modifica alla fonte. Su desktop la lista e
...
**Conservare.** I media WhatsApp arrivavano nel CRM come solo `mediaId`: i
byte si andavano a riprendere da Meta a ogni apertura, e Meta li scarta dopo
circa trenta giorni. Dall'08/09/2026 `conservaMediaWhatsApp` li scarica
appena il messaggio entra e li mette nello storage con `storageKey`, come già
faceva la posta. Regole: fuori dal percorso del webhook (che deve rispondere
in fretta), solo con storage durevole (col driver `local` su Railway
finirebbero in JSONB), tetto 15 MB per file, e un media che non si scarica
...
**Archiviare.** «Archivia nel fascicolo» (`mail.comunicazioni.archiviaAllegato`)
vale per i due canali: si scelgono la **commessa** e il **tipo**, e il file
entra col nome del tipo, il cliente e la data del messaggio (§8.4) — nome
mostrato prima di confermare. Se il messaggio non era collegato a nessuna
commessa lo diventa: dire di quale lavoro è un allegato lo dice anche del
messaggio che lo portava. Il fascicolo non duplica: stesso checksum (o stesso
nome d'origine e dimensione per i record legacy) significa stesso documento.
La procedura sostituisce `mail.email.archiviaAllegato`, che valeva solo per
la posta, archiviava sempre come «altro» e pretendeva che la mail fosse già
collegata alla commessa giusta.
...
Principio fissato dalla direzione: Wyndoor governa il lavoro del
...
`server/computo/__fixtures__/casi-reali.json` contiene **148 casi** (dal
06/09/2026 sera, fasi 1 e 5 dello studio sui dati reali): i 3 storici del
03/09, 17 dai fogli 2026, 57 dai fogli 2022-2025 sulla scrivania della
direzione e 71 dai fogli 2022-24 del Drive del NAS. **138 sono verdi al
centesimo**; **10 sono saltati** con il motivo scritto nel campo `salta` di
ciascun caso — divergenze capite e dichiarate, non tolleranza allargata
(tre fogli 2023 hanno i massimali a 1-2 € su ~10.000: tolleranza dichiarata
nel caso, non salto). Un foglio compilato con misure decimali (foro +
alette) non è riproducibile in mm interi: il raccoglitore lo dichiara
(`salta` H9) e non entra in fixture. Ogni caso porta l'**edizione** del listino
...
## 60. SaaS multi-azienda — modello commerciale e architettura (design approvato il 06/09/2026; WS1 e WS2 su `main` e in produzione dall'08/09/2026, WS3 e WS4 implementati su branch l'08/09 e non in produzione, WS5 e WS6 su `main` e in produzione dal 09/09/2026, workstream 7–8 non implementati)
...
(06/09/2026 sera):** il prodotto si chiamerà **Wyndoor**; dove qui e nella
spec si legge il vecchio nome del marchio vale «marchio Wyndoor» (spec §18-bis).
...
Wyndoor viene distribuito a più rivenditori di infissi come SaaS con
...
piattaforma è uno solo (nome, dominio, login, design system: **Wyndoor** dal
...
- La fattura del canone la emette il sistema contabile della società
  proprietaria di Wyndoor, separato dalle integrazioni FiC dei tenant.
...
Con la **prova gratuita di 30 giorni** (decisione del 06/09 sera, spec
§18-bis) il tenant nasce **alla registrazione**, in stato `trialing`, e il
checkout diventa il passaggio a pagante entro la scadenza: il passo «tenant
creato dopo la conferma del pagamento» resta solo come testo storico del
design approvato.
...
### 60.9 Workstream 1 — fondazione tenant (contratto implementato su branch il 07/09/2026, fuso in `main` con la PR #3 l'08/09/2026)
...
`docs/superpowers/plans/2026-09-06-ws1-fondazione-tenant.md`; runbook:
`docs/runbooks/multi-azienda.md`.

**Stato reale.** I 15 task sono implementati e committati sul branch
`feature/ws1-fondazione-tenant` (nato da `claude/ruffino-flow-saas-multi-afaecf`,
cioè `main` @ `ecb2042` più spec e piano), dal commit `4c3a71b` al `a01a757` (131 file, +3184/−193 fino al client), più i commit di
documentazione e correzioni finali `07f1b1f`…`e54dba4`.
Il branch non è su `main`: nessun push da qui. Il merge (= produzione, con
l'interruttore spento) resta una decisione della direzione, dopo una
verifica a schermo dell'utente.

**Revisione finale (07/09/2026).** Nessun Critical sull'intero branch;
alcuni Important, tutti corretti in un'unica onda: i ruoli vengono dallo
store a ogni richiesta e non dal JWT (`server/_core/context.ts`) — una
revoca vale alla richiesta successiva, non fra sette giorni; guardia
sull'ultima sede attiva del tenant (`sedi.update` con `attiva: false`);
`tenants.servizio.crea` resiliente a un commit fallito (l'evento `creato` si
registra subito dopo l'inserimento del tenant, non dopo la transazione di
sede/utente; sede e utente eventualmente spinti vengono tolti dagli array
vivi se il commit fallisce); hash della password azzerato dal payload del
comando `crea` alla sua chiusura (`server/tenants/repository.ts`); guardia
dell'ultimo proprietario applicata solo con `FLAG_MULTI_AZIENDA` acceso (con
l'interruttore spento il ruolo non è assegnabile, quindi non deve nemmeno
bloccare chi lo perde — comportamento di prima del WS1).
...
`protectedProcedure` è voluta; l'anteprima di `pnpm tenant` maschera già
l'hash della password (test in `cli.test.ts`); un `sedeId` nullo con
interruttore acceso è fermato dalla guardia «senza sede» e dal rifiuto delle
rotte Express. Corrette: §60.6 ora cita la prova gratuita; lo script
`pnpm tenant` non esegue più DDL (repository con `creaSchema: false`, sonda
`to_regclass`, si ferma se le tabelle mancano; test su Postgres vero).

**Minor lasciati dalla revisione (non bloccanti):** la riga di
`tenant.manage_proprietari` resta sovrascrivibile in `CapabilityMatrix`;
`contestoAutorizzazione` in `tars/strumenti/commesse.ts` è costruito a mano
senza tenant; lo script `scripts/tenant.ts` esegue `ensureSchema()`;
`RUOLO_COLORS` (client) non ha una voce per `proprietario`; un messaggio
letterale invece che da `MESSAGGI` in `permissions.ts`; un doppio guasto
possibile in `registraEvento` dentro il percorso d'errore del comando
(`comando_fallito`). Nessuno di questi ha una decisione registrata: vanno
trattati singolarmente, non nella prossima onda per inerzia.

Contratto in breve: **porta chiusa a chiave** (il tenant esiste, è nel
contesto, ha guardie; gli archivi business restano quelli di oggi e ogni
tenant diverso da 1 è rifiutato finché WS2 non apre la porta); tabelle
`tenants`, `tenant_eventi` (append-only garantito da trigger) e
`tenant_comandi`; `tenantId` su utenti e sedi con backfill a 1;
`proprietario` ottavo ruolo con `tenant.manage_proprietari`, l'unica
capability che la direzione non ha per costruzione; contesto `{ tenantId,
tenant, sedeId, sediIds }` con rilettura dell'utente a ogni richiesta (un
utente cancellato o disattivato perde subito la sessione); `sessionProcedure`
senza guardie solo per `tenants.mio`; `protectedProcedure` con porta chiusa,
sola lettura del tenant sospeso e sede attiva obbligatoria;
`assertTenantScope` sul control plane; Tars con `tenantId` obbligatorio e
senza fallback di sede; servizio di dominio `tenants` con comandi accodati
dallo script `pnpm tenant` ed eseguiti solo dal server (mai scritture esterne
con l'istanza viva); interruttore `FLAG_MULTI_AZIENDA` fail-closed, **spento
in produzione** (non esiste ancora nell'env Railway) = il CRM di oggi;
client toccato solo per l'etichetta «Proprietario». Deviazione dal design,
registrata nella spec: `portaChiusaPerTenant` vive in
`server/tenants/regole.ts` (pura), non in `servizio.ts`. Fuori: tutto ciò
che è dei workstream 2–6 e il rebranding Wyndoor.

**Verificato:** `pnpm check` verde a ogni task, `pnpm build` verde; suite
`vitest` verde a parte i 3 FAIL preesistenti e indipendenti dal WS1 (foto
HEIC vera via `sips`, legati ai binari della macchina); su Postgres vero
(Docker, usa-e-getta) schema idempotente, trigger append-only su
`tenant_eventi`, seed del tenant 1 con `setval`, `FOR UPDATE SKIP LOCKED`,
slug duplicato rifiutato anche a cache fredda; script `pnpm tenant` (i 4
sottocomandi, `--scrivi`/`--attendi`, `--anche-tenant-1`, password mai in
chiaro); boot locale in memoria con la riga di log atteso `[tenants] tenant 1
(ruffino-group) pronto: … utenti, … sedi`; pagina di login caricata nel dev
server senza errori console o di rete.

**Non verificato:** `/utenti` a 1440×900 e 390×844 (voce «Proprietario» nel
modulo utente) — serve il login demo, che l'agente non può digitare: da fare
dall'utente nell'anteprima; niente distribuito su Railway, dove
`FLAG_MULTI_AZIENDA` non esiste ancora nell'env (= spento) e nessuna verifica
in sola lettura è stata fatta in produzione; il comportamento con più
repliche (cache dei tenant e ciclo comandi sono in-process, una sola replica
come oggi).

**Prossimo passo:** verifica a schermo dell'utente, poi la decisione della
direzione sul merge su `main`.

### 60.10 Workstream 2 — porta aperta: archivi per tenant (contratto implementato su branch il 07/09/2026, fuso in `main` con la PR #5 l'08/09/2026)
...
assorbito `main`: PRD a **5.58**, rebranding Wyndoor nei documenti vivi
...
### 60.11 Workstream 3 — file, backup, credenziali e guasti per tenant (implementato su branch, 08/09/2026; non in produzione)

Spec tecnica approvata a sezioni in chat:
`docs/superpowers/specs/2026-09-08-ws3-file-integrazioni-design.md`; piano in
13 task: `docs/superpowers/plans/2026-09-08-ws3-file-integrazioni.md`;
runbook operativo: `docs/runbooks/multi-azienda.md`, sezione «WS3 — file,
...
revisione e una fix wave ha chiuso i quattro punti importanti che ne sono
usciti (giro notturno e interruttore, ricalcolo iniziale sul timbro,
documentazione dell'interruttore a flag spento, test pg del ricalcolo). Il
branch **non è su `main`**: nessun push, nessun merge da qui,
`FLAG_MULTI_AZIENDA` resta com'è. Il merge è una decisione della direzione.

**Che cosa cambia, per un'azienda.** Fino al WS2 gli archivi erano per
azienda, ma i file, il backup, le credenziali e i guasti erano ancora
dell'installazione: per questo il runbook vietava una seconda azienda in
produzione. Dal WS3:
- **I file sono contati per azienda.** Ogni file nuovo nasce sotto il
  prefisso della sua azienda (Ruffino Group compresa); i file già scritti
  restano dove sono e restano suoi. Un conto dei byte per azienda avvisa al
  50, all'80 e al 100 % di una quota (100 GiB di default) — **avvisa e
  basta**: nessun upload viene mai rifiutato. La quota che blocca è del WS4.
- **Ogni azienda ha il suo backup, sul suo Drive.** La direzione
  dell'azienda collega il proprio account Google da Integrazioni; il backup
  notturno scrive nella cartella dell'azienda («Backup Wyndoor —
  `<nome azienda>`»; Ruffino Group tiene la sua «Backup CRM Ruffino») e
  contiene **solo** i suoi dati, utenti compresi. Il refresh token è cifrato
  a riposo. Nessuna azienda vede il Drive di un'altra.
- **Il ripristino esiste ed è provato.** L'operatore può riportare gli
  archivi di un'azienda a un backup, prima in prova (il server legge il
  Drive e dice che cosa cambierebbe) e poi davvero, con l'azienda ferma
  durante la sostituzione. I file non si ricaricano: restano nello storage.
- **Le credenziali sono per azienda.** Gli `state` OAuth (Drive e Fatture in
  Cloud) sono persistiti e legati ad azienda, sede e utente; i messaggi
  WhatsApp si consegnano in base al **numero**, non alla firma, che con
  l'app condivisa non distingue più le aziende.
- **Un guasto di un'azienda non ferma le altre.** Un lavoro di fondo che
  fallisce tre volte di fila per un'azienda salta quell'azienda per 15, 30,
  60, 120 minuti, lo registra e riparte da solo; le altre continuano.
  Questa è **l'unica cosa del WS3 che si vede anche a interruttore spento**,
  con la sola Ruffino Group: un'integrazione rotta (Fatture in Cloud, posta,
  Tars, backup…) smette di riprovare a ogni giro e riparte dopo l'attesa,
  scrivendola nel log e in `pnpm tenant elenco`. Un riavvio riarma.
- **Un'azienda che non ha collegato il suo Drive viene saltata** dal backup
  notturno, con una riga di log: non è un errore e non rallenta le altre.
- **Un record che non esiste risponde «non trovato»**, non un errore del
  server: i 98 punti rimasti nei router sono stati uniformati.

**Che cosa fa l'operatore.** `pnpm tenant storage --slug=…` (conta, e con
`--ricalcola --scrivi` rifà il conto), `pnpm tenant ripristina --slug=…
--backup=… --prova|--scrivi`, `pnpm tenant elenco` (mostra anche i lavori
sospesi per guasto). L'ordine di rilascio e i messaggi d'errore sono nel
runbook.

**Verificato:** `pnpm check`, `pnpm test` (3131 test verdi, suite intera) e
`pnpm build`; test su Postgres vero per ledger, ricalcolo e ripristino; due
aziende con Drive finti nei test del backup. **Non verificato:** nulla è
stato distribuito, nessuna prova su Railway, nessun collegamento OAuth reale
di una seconda azienda, nessuna verifica a schermo (il WS3 non tocca il
client).

**Che cosa resta al WS4.** Abbonamenti, prova gratuita e stati di
pagamento; la quota che **blocca** invece di avvisare, con tolleranze e
pacchetti extra; il **budget Tars per azienda** (oggi i tetti sono somme
globali: un'azienda può esaurirli per tutte); l'export aziendale.
Onboarding, inviti e personalizzazione restano al WS5, il pannello Platform
Admin al WS6.
...
### 60.12 Workstream 4 — abbonamenti (implementato su branch, 08/09/2026; non in produzione)

Spec: `docs/superpowers/specs/2026-09-08-ws4-abbonamenti-design.md` (le
decisioni d'esecuzione, venti, sono nella sua §2-bis); piano in 9 task:
`docs/superpowers/plans/2026-09-08-ws4-abbonamenti.md`; runbook:
`docs/runbooks/multi-azienda.md`, sezione «WS4 — abbonamenti, quota che
blocca, budget Tars per azienda». I 9 task sono committati su
`feature/ws4-abbonamenti` (`bb2f147`…`d335a68`), nato dal branch del WS3:
finché la PR #7 non è fusa, la PR del WS4 conterrà anche il WS3. La revisione
dell'intero branch ha poi prodotto una **fix wave finale** (09/09/2026,
`fa5b8be`…`3a5cf0b`): la suite era rossa per un orologio non finto in un
blocco di test, e ne sono usciti tre ruling — R15 (l'abbonamento è la fonte di
verità: un contratto sospeso o disdetto con l'azienda attiva viene risospeso
dal giro successivo), R16 (il tenant 1 è esente dal blocco dello spazio come
già dal tetto Tars) e R17 (la migrazione dei record legacy rispetta il blocco
e si ferma subito col messaggio della quota). Il branch **non è su `main`** e
`FLAG_MULTI_AZIENDA` resta com'è: il merge è una decisione della direzione.

**Che cosa ha un'azienda, dal WS4.** Un contratto, non solo un archivio.
Nasce con **30 giorni di prova**, 25 €/mese di Tars incluso e 7 giorni di
tolleranza su spazio e Tars (§18-bis della spec madre). Riceve un avviso a 7,
3 e 1 giorno dalla scadenza; poi va in **insoluto** e, dopo altri 7 giorni, in
...
— `--omaggio [--scadenza=…]`, `--proroga=<giorni>`, `--quota-gb=<n>`,
`--budget-tars-eur=<n|nessuno>`, `--extra-tars-eur=<n>`,
`--tolleranza-storage=<gg>`, `--tolleranza-tars=<gg>`,
`--disdetta`/`--annulla-disdetta` — e `pnpm tenant elenco`, che stampa stato,
scadenza e insoluto di ogni azienda. Ruffino Group rifiuta omaggio, proroga e
disdetta: è la proprietaria della piattaforma, e il suo abbonamento omaggio
senza scadenza nasce da solo al primo boot. **A interruttore spento non
cambia niente**: nessun worker, nessun blocco, nessun tetto per azienda.

**Verificato** dopo la fix wave (09/09/2026): `pnpm check` (`tsc --noEmit`)
pulito, `pnpm test` 3276 test verdi su 313 file (78 saltati), `pnpm build`
riuscito; i test su Postgres vero — control plane e ledger dei costi con due
aziende — rieseguiti a parte (19 casi in tre file). Interfaccia a 1440 e 390
col login demo, verificata alla chiusura del Task 9. **Non verificato:** nulla
distribuito, nessuna prova su Railway, nessun pagamento reale.

**Che cosa aspetta ancora.** Il **provider di pagamento** — checkout, portale
del proprietario, webhook, fattura del canone — entra nell'adattatore
`server/abbonamenti/provider.ts`, oggi con la sola implementazione «nessuno»;
il dominio consuma già eventi normalizzati e idempotenti. Registrazione
self-service, inviti e personalizzazione restano al **WS5**; pannello Platform
Admin — che userà questi stessi comandi — ed export aziendale al **WS6**.
Prezzo del canone, budget incluso e prezzo degli extra restano decisioni della
direzione: nel codice sono configurazione (`SAAS_BUDGET_TARS_EUR_MESE`,
`SAAS_CAMBIO_EUR_USD`) e comandi.

**Prossimo passo:** decisione della direzione sul merge dei branch WS3 e WS4,
poi il **WS5** — onboarding self-service (spec e piano già scritti in un altro
worktree da un'altra sessione: da coordinare prima di partire).

> **Aggiornamento 09/09/2026:** i branch WS3 e WS4 sono stati fusi in `main`
> (PR #8, merge `37c1889`) e `FLAG_MULTI_AZIENDA` è stato acceso in
> produzione lo stesso giorno alle 09:54 (Europe/Rome). Il WS6, sotto,
> riparte da questo `main`.

### 60.13 Workstream 6 — pannello piattaforma (su `main` e in produzione dal 09/09/2026)

Spec: `docs/superpowers/specs/2026-09-09-ws6-pannello-piattaforma-design.md`
(le decisioni d'esecuzione, tredici, sono nella sua §2-bis); piano in 10 task:
`docs/superpowers/plans/2026-09-09-ws6-pannello-piattaforma.md`; runbook:
`docs/runbooks/multi-azienda.md`, sezione «WS6 — pannello piattaforma». I 10
task sono committati su `feature/ws6-pannello-piattaforma`
(`9dc4c54`…`a606515`), nato da `main` @ `37c1889` — cioè dal `main` in cui
WS3 e WS4 sono già dentro e in produzione; sopra di loro, la fix wave finale
uscita dalla revisione dell'intero branch (`e59d8ed`…, ruling R9-R11).

**Che cosa ha in più chi amministra la piattaforma.** Una sezione
`/piattaforma` nel CRM: elenco di tutte le aziende con stato, abbonamento,
spazio, Tars del mese, worker sospesi, ultimo backup e proprietari; «Nuova
azienda» con sede e proprietario, che manda subito l'invito; la scheda di
un'azienda con abbonamento (le sette azioni: omaggio, proroga, quota, budget
Tars, extra Tars, tolleranze, disdetta), spazio, Tars, proprietari e inviti,
backup e ripristino, eventi e comandi. Ogni scrittura passa da
`tenant_comandi`, eseguita **subito** dalla stessa funzione del giro dei 30
secondi (ricalcolo dello spazio e ripristino restano in coda, perché è il
server a dover parlare col Drive dell'azienda); ogni evento porta l'attore
`piattaforma:<email>`.

**Le decisioni della direzione (spec §2, 09/09/2026).** (1) **Stesso utente,
potere in più**: chi amministra è un utente attivo del tenant 1 la cui email
è in `PLATFORM_ADMIN_EMAILS` — **deviazione dichiarata** dal design madre
(identità separata con MFA, §6.2/§15/§18 di
`2026-09-06-saas-multi-azienda-design.md`): oggi la piattaforma ha una sola
persona che la amministra ed è anche la proprietaria del tenant 1, e un
secondo login con TOTP da costruire è un costo senza beneficio finché non
esiste un secondo amministratore. Resta del design: la capacità non si
assegna dall'app, il pannello non vede dati di dominio, ogni azione è
tracciata, le azioni sensibili chiedono la password. La MFA arriva come passo
...
**Le tredici decisioni d'esecuzione (spec §2-bis).** Due prima del Task 1
(l'ordine con cui il Task 8 e il Task 9 si dividono la rotta
`/piattaforma/:slug`; `passwordSchema` di `utenti.ts` esportato) e otto dalla
revisione dei task: **R1**, le tre letture in blocco di Postgres
(`comandiDi`, `storageTutti`, `eventiRecenti`) non erano mai state esercitate
contro un database vero — tre test aggiunti, nessun bug trovato; **R2**, il
repository in memoria non aveva un claim sul comando in esecuzione, quindi
nessun test in memoria poteva provare l'attesa di `eseguiComandoSubito` — un
insieme `inEsecuzione` aggiunto (su Postgres il comportamento era già
corretto, grazie a `FOR UPDATE SKIP LOCKED`); **R3**, `utenteAmministratore`
e `tenants.mio` non controllavano `loginMethod`, quindi un utente OAuth
legacy con lo stesso id numerico di un amministratore sarebbe stato trattato
come tale — corretto con lo stesso controllo di `risolviTenantPerUtente`
(non sfruttabile oggi, OAuth è spento); **R4**, un errore inatteso
nell'accettazione di un invito spariva in silenzio dietro lo stesso
`NOT_FOUND` di un token sbagliato — ora logga il messaggio (mai il token)
prima di rilanciare la stessa risposta; **R5**, `ultimoBackup`/`backup`
portano il record intero di `BackupLog`, non il sottoinsieme di campi dello
sketch — nessun segreto, annotato in §5.1; **R6**, `ripristina` con
`scrivi: true` — l'unico ramo sensibile del comando — non aveva alcun test
sul cancello della password, chiuso con test mirati; **R7**,
`NuovaAziendaDialog` mostrava «Azienda creata» anche quando il comando era
finito in errore — ora titolo, descrizione e «Apri la scheda» seguono lo
stato vero del comando; **R8**, la scheda seguiva un solo comando lungo alla
volta, e un secondo comando (ricalcolo o ripristino) avviato mentre il primo
era in corso gli faceva perdere polling e toast — corretto con una mappa per
id, un componente di polling per comando. Nessuna di queste otto ha richiesto
di riscrivere una decisione della direzione: sono tutte lacune di copertura o
di comportamento trovate durante l'esecuzione, chiuse nello stesso branch.

Le ultime tre vengono dalla revisione finale del branch intero, chiuse in
un'unica fix wave. **R9** — e questa sì corregge la spec: §3.2 metteva
`invita` fra le azioni non sensibili, ragionando sul fatto che non cambia
nulla nei dati; ma un link d'invito è la presa dell'account del proprietario
di un'altra azienda, e chi ce l'ha sceglie la password ed entra. Ora `invita`
chiede la conferma della password come le altre, e il `link` torna al browser
**solo** quando la posta non è partita: se l'email è partita il token è già
nella casella giusta, e una seconda copia nella pagina dell'amministratore
sarebbe soltanto un'altra copia da rubare. **R10**, la porta chiusa a
interruttore spento: il runbook consigliava `FLAG_MULTI_AZIENDA=off` come
rollback, ma a flag spento il contesto fissa `tenantId = 1` per chiunque e
l'utente di un'altra azienda finirebbe dentro Ruffino Group con tutte le sue
sedi. Il difetto era già su `main`; il WS6 lo rende raggiungibile, perché
creare una seconda azienda ora è un dialogo invece di una riga di comando.
Ora `auth.login` rifiuta con «Accesso non disponibile: il multi-azienda della
piattaforma è spento.» (dopo la verifica della password, così non si
enumerano le email delle altre aziende), `createContext` non apre quella
sessione e la pagina d'invito rifiuta senza consumare il token; il runbook
dice che spegnere l'interruttore vale finché l'azienda è una sola — per
fermarne una, si sospende. **R11** raccoglie il resto: banner del runbook
aggiornati e sezione WS6 al posto giusto, un avviso al boot se `APP_BASE_URL`
manca (con la base mostrata accanto al link d'invito), la scheda che si
rilegge ogni 60 s invece di 15 dichiarando quanto costa, e sette minori — la
costante `TENANT_PIATTAFORMA_ID` in un modulo solo, `key={slug}` sulla rotta
della scheda, la password azzerata da «Riprova», l'indice parziale unico
`tenant_inviti_valido_idx` con un ritentativo in `emettiInvito`, `adesso`
onorato dal driver Postgres degli inviti, il limite di tentativi anche
sull'anteprima dell'invito, `payloadSenzaSegreti` sui comandi che il pannello
restituisce.

**Le variabili d'ambiente.** `PLATFORM_ADMIN_EMAILS` (obbligatoria per vedere
...
gated dall'identità, non da un interruttore nuovo (spec §3.3).

**Verificato (09/09/2026, dopo la fix wave finale):** `npx vitest run
shared/brand.test.ts` 4/4; `pnpm check` (`tsc --noEmit`) pulito; `pnpm test`
(suite intera, senza `DATABASE_URL`) 339 file passati + 15 saltati (354),
3665 test passati + 86 saltati (3751), zero falliti; `pnpm build` riuscito
(client e server, solo il warning preesistente sulla dimensione di
`dist/index.js`); `DATABASE_URL=… npx vitest run pg.test
--no-file-parallelism` contro `perf-pg-test` (control plane, ledger dei costi
e gli altri 12 file `*.pg.test.ts`): 14 file passati, 71 test passati, zero
falliti. Interfaccia a 1440×900 e 390×844
verificata dai Task 8 e 9 montando le pagine con un link tRPC finto e dati
seminati (screenshot in `.superpowers/sdd/2026-09-09-ws6-pannello-piattaforma/task-{8,9}-screens/`),
non attraverso `/piattaforma` con una sessione autenticata vera: il pannello
Browser di quelle sessioni non aveva un cookie di sviluppo, e firmare a mano
un token o digitare la password demo sono entrambi fuori dalle regole
dell'agente. **Non verificato:** `/piattaforma` con una sessione reale
(guardia, voce di menu, riga → scheda, ogni mutation contro il server vero,
il giro completo dei comandi lunghi); nessuna consegna reale via Resend
(account e dominio `wyndoor.com` non ancora verificati); nulla distribuito su
Railway.

**Che cosa resta.** Il **provider di pagamento** (Stripe o equivalente):
resta fuori, l'adattatore del WS4 resta «nessuno» — il webhook futuro
chiamerà lo stesso `crea` e lo stesso `invitaProprietario` di questa spec
(§13). **MFA** per amministratori e proprietari, **accesso di supporto**
(impersonazione con motivazione, scadenza e audit) e **reset password
self-service**: fuori, come dichiarato dal perimetro (spec §1) — la tabella
`tenant_inviti` nasce già con un campo `tipo` per accoglierli. I minori
lasciati aperti sono elencati nella spec §2-bis, «Ancora aperti», e nessuno
blocca il rilascio: qualche lacuna di copertura, un caso limite mai prodotto
dal server, e quattro dichiarati dalla revisione finale — il token
dell'invito che viaggia nella query string (`anteprima` è una query: farne
una mutation è un cambio di contratto lato client), `RequirePiattaforma`
senza test DOM (non esiste un harness `.test.tsx` nel repository),
disattivare un utente non annulla i suoi inviti, e `tenants.mio` chiesto a
ogni utente dal menu solo per decidere se mostrare la voce «Piattaforma».

**Prossimo passo:** verifica a schermo con una sessione reale, poi la
decisione della direzione sul merge in `main`; dopo, Resend vero (account,
dominio, chiave) prima di invitare la prima azienda pilota fuori da un
ambiente di prova.

> **Aggiornamento 09/09/2026 (sera):** il branch è stato fuso in `main`
> (PR #9, merge `cff8ef0`) e distribuito: il WS6 è **in produzione**. Il
> WS5, sotto, nasce da questo stesso `main`.

> **Addendum «Modifica azienda» (decisione della direzione, 09/09/2026
> sera; spec §15).** Dopo la creazione dal pannello, un'azienda si
> corregge: ragione sociale, slug, note; i sei campi di fatturazione
> (P.IVA, codice fiscale, sede legale, email amministrativa, PEC, codice
> SDI — colonne additive su `tenants`); la sede predefinita (nome, città);
> il proprietario (nome, cognome, email, telefono). Due comandi nuovi in
> `tenant_comandi`, eseguiti **subito** come gli altri (§5.2 della spec):
> `modifica_tenant` (evento `tenant_modificato` con
> `{ campi: [{ campo, prima, dopo }] }`, solo i campi davvero cambiati;
> `slug_cambiato` con `{ da, a }` se lo slug cambia davvero; il tenant 1
> non cambia slug) e `modifica_proprietario` (evento
> `proprietario_modificato` con `{ utenteId, campi }`; l'email resta unica
> su tutta l'installazione, case-insensitive). Se l'email del proprietario
> cambia e per lui c'è ancora un invito valido, il router lo annulla
> (`invito_annullato`) e ne emette uno nuovo (`invito_inviato`, stessa
> regola R9 sul link) — **solo per quel proprietario**: un'azienda con più
> proprietari non tocca l'invito di un altro. `accettaInvito` rifiuta
> comunque (stesso esito generico di un token scaduto) un link la cui
> email non coincide più con quella corrente dell'utente, per la finestra
> in cui l'email cambiasse fuori dal router. Nel pannello: pulsante
> «Modifica» nella scheda, dialogo a quattro pannelli (Azienda,
> Fatturazione, Sede, Proprietario) con una sola conferma password; da
> script, `pnpm tenant modifica --slug=… [--nome=…] [--nuovo-slug=…]
> [--piva=…] [--cf=…] [--sede-legale=…] [--email-amministrativa=…]
> [--pec=…] [--sdi=…] [--note=…] [--scrivi] [--attendi]` (copre solo dati
> e fatturazione, non sede né proprietario). **Verificato:** `pnpm check`
> pulito, `pnpm test` 355 file passati + 15 saltati (3864 test passati +
> 88 saltati), `pnpm build` riuscito, i quattro `*.pg.test.ts` di
> `server/tenants` 4 file / 26 test su Postgres vero. Su branch
> `feature/modifica-azienda`, PR aperta: il merge è una decisione della
> direzione.

> **Addendum 10/09/2026 — la mail d'invito (§60.13.2).** Fino a oggi
> l'invito partiva come cinque `<p>`: nessun `<html>`, nessuna codifica
> dichiarata, nessun contenitore, il link come testo di sé stesso. Fuori
> da Gmail quella non è una mail di prodotto — Outlook desktop, che rende
> con il motore di Word, la mostrava in Times New Roman a tutta larghezza.
> È la **prima** cosa che un'azienda cliente vede di Wyndoor.
>
> Ora la forma vive in `server/_core/bustaEmail.ts` (`componiEmail`,
> funzione pura): documento HTML `lang="it"` con codifica e
> `color-scheme: light dark`; tabelle e larghezza **fluida con tetto a
> 600 px** (il 600 fisso solo dentro una tabella condizionale `[if mso]`,
> altrimenti su un telefono la mail esce di lato); bottone «bulletproof»
> con la variante VML per Outlook; marchio da `<base>/marchio-email.png`
> (PNG **trasparente** nuovo, generato da `pnpm icone` accanto alle icone
> iOS che invece nascono su fondo pieno) con `alt=""`, perché la parola
> «Wyndoor» gli sta accanto come testo vivo e un client che blocca le
> immagini non deve scrivere «Wyndoor Wyndoor»; preheader nascosto; tema
> scuro con la palette Modular Control. **HTML e testo semplice nascono
> dalla stessa struttura**: due funzioni separate divergono al primo
> ritocco. La busta è condivisa di proposito — le prossime mail di
> piattaforma (scadenza della prova, abbonamento sospeso) non devono
> reinventare marchio, bottone e piede.
>
> Le parole restano in `server/piattaforma/testi.ts`, che ora non contiene
> un solo tag. Oggetto orientato all'azione: «Attiva il tuo accesso a
> Wyndoor per `<Azienda>`» (i primi 40 caratteri sono tutto ciò che si
> vede su un telefono). Nel corpo una scheda con le tre cose che il
> destinatario non ricorderà: **quale azienda**, **con quale indirizzo
> entra** (è il suo nome utente, prima non veniva detto) e **fino a
> quando** vale il link — una **data per esteso** («fino al 17 settembre
> 2026», fuso `Europe/Rome`), non «fra 7 giorni», che è vero solo il
> giorno in cui la mail parte. Un secondo paragrafo dice che cosa succede
> dopo il click, perché l'accettazione atterra su
> `/integrazioni?attivazione=1` e nessuno lo aveva mai scritto.
>
> Il mittente resta un `no-reply`: `POSTA_PIATTAFORMA_RISPOSTA` (nuova,
> facoltativa) diventa il `Reply-To:` e il contatto stampato nel piede.
> Senza, la mail non promette nessun contatto — meglio niente di un
> indirizzo morto.
>
> **Pannello.** `testoEsitoInvito` riceveva già `motivo` dal server
> (`esitoPubblico`) e lo buttava via: un rifiuto di Resend (422 su un
> dominio non verificato, un timeout, la rete) si leggeva come «Posta
> della piattaforma non configurata», e chi guardava andava a cercare su
> Railway una chiave che c'era già. Ora i due casi si distinguono, e il
> riquadro dell'esito è **ambra con un triangolo** quando la posta non è
> partita, **verde con una busta** quando è partita: prima erano due
> grigi identici, e la differenza è l'unica cosa che conta — se la posta
> non è partita, quel link è l'unica copia che esiste.
>
> **Pagina `/invito/:token`.** Saluto per nome (`nome` arrivava
> dall'anteprima e non veniva mostrato) e la stessa scheda «Azienda /
> Accesso» della mail: chi ha appena premuto un bottone in una casella
> deve riconoscere il posto. Le due regole della password si accendono
> **mentre si scrive** invece di comparire come errore dopo l'invio
> (`regolePassword`/`erroreInvioPassword` in
> `client/src/pages/piattaforma/testi.ts`, moduli puri: la suite gira
> senza DOM e non potrebbe provarle dentro il componente).
>
> **Verifica.** Una mail non si guarda in un test: `pnpm posta:anteprima`
> (`scripts/anteprima-email.ts`) la scrive su file, con `--base`,
> `--contatto` e `--tema=scuro` (che forza la media query — è una
> simulazione, non una prova: quella è aprirla in Apple Mail). Controllata
> a 1440 e 390, chiara e scura; il giro completo — creazione azienda →
> link → `/invito` → password → `/integrazioni?attivazione=1` — provato
> dal vivo su un server locale con `FLAG_MULTI_AZIENDA` acceso. `pnpm
> check` pulito, `pnpm test` 356 file passati + 15 saltati (3918 test
> passati + 88 saltati), `pnpm build` riuscito.

### 60.14 Workstream 5 — collegamento delle integrazioni in self-service (su `main` e in produzione dal 09/09/2026)

Spec: `docs/superpowers/specs/2026-09-08-ws5-collegamento-integrazioni-design.md`
(le decisioni d'esecuzione, quattro ruling più i minori, sono nella sua
§2-bis, «Ancora aperti» compreso); piano in 14 task:
`docs/superpowers/plans/2026-09-08-ws5-collegamento-integrazioni.md`;
runbook: `docs/runbooks/multi-azienda.md`, sezione «WS5 — collegamento delle
integrazioni in self-service». I 13 task di codice sono stati eseguiti
inline (senza revisione per task) su `feature/ws5-collegamento-integrazioni`,
un altro worktree con base il WS4 a metà (`873b6c6`), poi fuso con il WS4
vero (`e38874c`). Quel branch è stato fuso in
`feature/ws5-integrazioni-su-main` (nato da `main` @ `cff8ef0` — che
contiene già WS1, WS2, WS3, WS4 e WS6, con `FLAG_MULTI_AZIENDA` acceso in
produzione — merge `c139b83`), poi sottoposto a una revisione dell'intero
branch e a un'unica fix wave (8 commit, `2901017`…`0d6d70d`).
...
registro (`server/integrazioni/registro.ts`) conta però **cinque**
adattatori: `calendario` resta un tipo senza implementazione, perché l'OAuth
Google in entrata è la fase 4 (§11 della spec), non partita. Ogni adattatore
**avvolge** il router che già esisteva — nessuno riscritto — e dichiara lo
stesso permesso che quel router applica già; `stato()` non chiama mai il
fornitore (regge sei volte il caricamento della pagina Impostazioni),
`verifica()` è l'unica prova viva, su richiesta, in cache 60 secondi per
azienda. Le credenziali OAuth (Fatture in Cloud, Google) e l'app Meta di
WhatsApp diventano **di piattaforma**: il cliente autorizza sul proprio
account, ma non apre mai un'area sviluppatori — per WhatsApp questo toglie
l'unico passo che oggi chiede un App ID a un titolare. Il percorso di
attivazione guidato (`client/src/integrazioni/PercorsoAttivazione.tsx`) è
raggiungibile dopo l'invito (WS6): il proprietario ci atterra da solo
(`?attivazione=1`), con «Salta» sempre disponibile e un richiamo in cima
alle Impostazioni finché resta un passo da fare.

**Le otto decisioni della direzione (spec §2, 08/09/2026).** (1) Il cliente
collega da solo, senza assistenza. (2) App di piattaforma ovunque esista
OAuth: l'app chiede, l'account del cliente concede. (3) L'email resta IMAP
com'è — niente OAuth, niente verifica Google «restricted». (4) I calendari
entrano via OAuth di piattaforma, accanto alle sorgenti iCal esistenti che
restano valide. (5) Il calendario in **scrittura** (CRM → Google) è
approvato ma resta una spec propria — non in questo workstream. (6) Fatture
in Cloud chiede sola lettura all'attivazione e la scrittura al primo
bisogno, spiegata nel momento in cui serve. (7) Approccio A: cornice unica,
sei adattatori — non solo le pratiche esterne, non si parte dal percorso
guidato da solo. (8) Gli adattatori avvolgono i router di WS3, non li
riscrivono; PRD e handoff si scrivono per ultimi (questa sezione).

**Le decisioni d'esecuzione (spec §2-bis).** La revisione dell'intero branch
(`cff8ef0..c139b83`) ha trovato **2 Critical e 11 Important**, chiusi in
un'unica fix wave con quattro ruling: **W2 (C2)**, il redirect OAuth del
Drive viaggia ora nel payload dello `state` (`oauth_state`, tipo `gdrive`)
come già fa Fatture in Cloud — prima l'avvio usava il callback canonico e lo
scambio del codice lo ricostruiva dall'header `Host`, due redirect diversi
nello stesso giro, cioè `redirect_uri_mismatch`; **W3 (I6, I7)**, «Prova il
collegamento» (`verifica()`) e il percorso di attivazione dovevano essere
raggiungibili subito, non rimandati: fatto, con un richiamo in cima alle
Impostazioni; **W4 (I9)**, il modulo manuale di WhatsApp resta sempre
raggiungibile dentro una Diagnostica, anche con l'app di piattaforma pronta
— sparire al primo deploy avrebbe tolto a Ruffino Group l'unica strada di
chi ha già collegato un numero a mano; **W5**, i minori a buon mercato
(etichette centralizzate, invalidazione della cache, `useSearch`,
`assertChiaveCifratura` in testa a `fic.avvia`, `WHATSAPP_VERIFY_TOKEN` di
piattaforma) entrano nella stessa tornata. L'altro Critical, **C1**, era la
cache di `verifica()` con una chiave senza l'azienda: gli adattatori ad
ambito azienda (`backup`, `agente`) finivano in una voce sola per tutta
l'installazione, e l'esito della prova di un cliente veniva servito a un
altro — corretto includendo `tenantId` nella chiave e invalidando la voce a
ogni `avvia`/`completa`/`scollega`. Cinque minori restano rimandati ai
documenti (spec §2-bis, «Ancora aperti»; runbook, «Debiti dichiarati»): il
soggetto di Fatture in Cloud mostra l'id e non il nome dell'azienda,
`agente.verifica()` controlla solo che la chiave OpenAI sia presente e non
interroga il provider, tre ripieghi `?? 1` invece del riferimento a
`DEFAULT_SEDE_ID`, il ramo service-account del Drive (debito preesistente,
da togliere prima della verifica Google), e `getCfg` che scrive una riga
nuova quando la chiama uno `stato()` nominalmente di sola lettura.

**Le variabili d'ambiente.** `FIC_OAUTH_CLIENT_ID`/`FIC_OAUTH_CLIENT_SECRET`/
`FIC_OAUTH_REDIRECT_URI` sono già impostate su Railway (l'app OAuth di
piattaforma esiste, privata con whitelist fino a 20 email — la sede 2 di
Ruffino Group è già collegata via OAuth con scrittura, la sede 1 resta a
token manuale). `GOOGLE_OAUTH_REDIRECT_URI`
...
dall'handshake del webhook insieme ai token già generati per sede, ma non è
ancora impostata: il webhook a livello di app (già puntato su
`https://app.wyndoor.com/api/webhook/whatsapp`) valida oggi solo perché usa
un token già presente nel CRM.

**Verificato (09/09/2026, dopo la fix wave):** `npx vitest run
shared/brand.test.ts` 4/4; `pnpm check` (`tsc --noEmit`) pulito; `pnpm test`
(suite intera, senza `DATABASE_URL`) 351 file passati + 15 saltati (366),
3759 test passati + 86 saltati (3845), zero falliti; `pnpm build` riuscito
(client e server, solo il warning preesistente sulla dimensione di
`dist/index.js`); `DATABASE_URL=… npx vitest run pg.test
--no-file-parallelism` contro `perf-pg-test`: 14 file passati, 71 test
passati, zero falliti. **Non verificato:** nessun giro OAuth reale (Fatture
in Cloud, Google), nessuna chiamata a Meta, nessun Embedded Signup con
l'app di piattaforma, nulla distribuito su Railway con queste variabili; la
fix wave ha verificato l'interfaccia a 1440×900 e 390×844 montando la
pagina Impostazioni vera con un link tRPC finto (screenshot in
`.superpowers/sdd/2026-09-08-ws5-collegamento-integrazioni/fix-wave-screens/`),
non attraverso `/integrazioni` con una sessione autenticata vera — stesso
limite già dichiarato da WS4 e WS6: il pannello Browser di queste sessioni
non ha un cookie di sviluppo.

**Che cosa resta.** Il **calendario in entrata** (fase 4, OAuth Google):
dipende dalla verifica del consent screen Google (fase 0) e dall'aggiunta di
`'gcal'` al `CHECK` di `oauth_state`, oggi ancora `('fic','gdrive')`. Il
**calendario in scrittura** (fase 5): spec propria, non scritta. La
**richiesta di app pubblica** a Fatture in Cloud: non ancora partita, quindi
il collegamento resta dietro la whitelist dell'app privata. La
semplificazione di `mittenteWebhookWhatsApp` (che con un segreto di
piattaforma non avrebbe più bisogno di provare il segreto di ogni azienda):
debito dichiarato su un file di WS3, fuori da questo workstream.

**Prossimo passo:** impostare `GOOGLE_OAUTH_REDIRECT_URI` e la terna
...
### 60.15 Staging con dati demo e accesso di prova permanente (10/09/2026; su branch, ambiente Railway NON ancora creato)

Requisito: ogni modifica UI deve potersi verificare a schermo, contro un
server vero, prima del merge — non solo nei test automatici. Il runbook
multi-azienda nomina «staging» in quattro punti come luogo dove far nascere
le aziende di prova, ma l'ambiente non era mai stato creato. Spec:
`docs/superpowers/specs/2026-09-10-staging-demo-design.md` (decisioni
D1–D8); piano in 8 task:
`docs/superpowers/plans/2026-09-10-staging-demo.md`; runbook operativo:
`docs/runbooks/staging.md`.

Tre meccanismi, tutti dietro l'identità d'ambiente `AMBIENTE=staging`
(mai `NODE_ENV`, che deve restare `production` anche in staging perché i
gate di sicurezza esistenti — segreto JWT, password di bootstrap — scattano
solo su quel valore esatto; `ambienteStaging()`,
`server/_core/ambiente.ts`): (1) **dati demo** seminati da soli al primo
avvio, solo su store `clienti`/`commesse` entrambi vuoti
(`server/staging/semeDemo.ts`: 6 clienti e 6 commesse, sempre attraverso i
percorsi di dominio esistenti, mai record costruiti a mano); (2) **accesso
di prova permanente**, una rotta anonima `GET /api/staging/entra?token=…`
che apre la sessione dell'utente demo senza digitare una password, doppio
gate (`AMBIENTE=staging` + token ≥ 32 caratteri) e 404 opaco su qualunque
fallimento (`server/staging/rotta.ts`); (3) **ambienti per PR**, la stessa
base sotto una levetta Railway che forka l'ambiente `staging` — nessun
codice in più. Un banner servito dal server (`system.ambiente`) avvisa in
entrambe le shell che l'ambiente è di prova.

Vincolo non negoziabile: **staging non parla mai con servizi esterni**.
I quattro giri di fondo che toccano IMAP, Fatture in Cloud/SdI e Google
Drive non partono in staging (`avviaGiriEsterni()`,
`server/_core/giriEsterni.ts`): così anche un futuro ripristino di un
backup di produzione in staging (prova di ripristino mensile) resta
inerte, invece di scrivere su servizi veri entro un minuto dal boot.

**Verificato:** `pnpm check`/`test`/`build` verdi; verifica browser
completa (accesso dal link col token, banner visibile, 1440×900 e
390×844 puliti, console senza errori, 404 su token errato o assente).
**Non fatto:** l'ambiente Railway non è stato creato da questo lavoro — la
creazione dell'environment `staging` con un Postgres dedicato, le
