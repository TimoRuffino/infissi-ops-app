...
## 60. SaaS multi-azienda — modello commerciale e architettura (design approvato il 06/09/2026; WS1 e WS2 su `main` e in produzione dall'08/09/2026, WS3 implementato su branch l'08/09 e non in produzione, workstream 4–8 non implementati)
...
dopo la fusione di WS1 e WS2. Il branch **non è su `main`**: nessun push,
nessun merge da qui, `FLAG_MULTI_AZIENDA` resta com'è. Il merge è una
...
**Verificato:** `pnpm check`, `pnpm test` (3114 test, suite intera verde) e
`pnpm build`; test su Postgres vero per ledger e ripristino; due aziende con
Drive finti nei test del backup. **Non verificato:** nulla è stato
distribuito, nessuna prova su Railway, nessun collegamento OAuth reale di
una seconda azienda, nessuna verifica a schermo (il WS3 non tocca il
client).
